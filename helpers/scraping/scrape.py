...
SHOW = re.compile(r'.*#(\d{1,6}) - (?:Monday|Tuesday|Wednesday|Thursday|Friday|Saturday|Sunday), (January|February|March|April|May|June|July|August|September|October|November|December) (\d{1,2}), (\d{4})')
MONTHS = {
    u'January': u'01',
    u'February': u'02',
    u'March': u'03',
...
if __name__ == u'__main__':
    ...
    for url in games:
        print(u'({completed}/{remaining}) Getting game id: {url}'.format(
            completed = str(len(pulled_games)).zfill(4),
            remaining = str(len(games)).zfill(4),
            url = url))
        try:
            ...
            print(u'An error occured with {url}'.format(url = url))
            continuity = input(u'Would you like to (s)ave the existing files, (a)dd the link to an \'error\' list, or (q)uit without doing anything? ').lower()
